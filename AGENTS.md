...
- Split when the child's resolved identity differs and the child is a real app.
  Before `user_place`, `spawned_app` bills a command to the Applications
  ancestor that spawned it when a shell, launcher, generic or noise wrapper
  sits in between. The walk stops at a terminal, compositor or systemd, so
  the same binary from a terminal stays its own row. An app-rule process that
  exec'd this one directly does not absorb it (cursor's agent launches
  claude; claude stays its own row). A unique non-noise payload child (claude,
