...
//! clock, so an idle table costs one image a sample. That is affordable here
//! for a reason it would not be there: a sparkline is mostly empty, and sixel
//! run-length-encodes the empty part rather than sending the whole bitmap.
